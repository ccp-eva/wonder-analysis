...
```{r}
item_difficulty <- ranef(irt1_final)$word%>%as_tibble(rownames = "word")

item_difficulty%>%saveRDS("../saves/easiness_final.rds")
```
...
p_icc <- icc1_final %>% 
  ...
  theme_minimal()+
  theme(legend.background = element_blank(), legend.position = c(0.1, 0.8), legend.direction = "h")
```
...
```{r}
ggdraw() +
  draw_plot(p_icc + theme(axis.title.y = element_blank(), axis.text.y = element_blank(), axis.ticks.y = element_blank(), legend.position = c(0.1, 0.1)))+
  draw_plot(p_tic + theme(plot.background = element_blank()), x = 0.7, y = .2, width = .42, height = .42)
```

```{r}
ggsave("../graphs/ICC_1PL_final.pdf", height = 6, width = 12, scale = 1, bg = "white")
```
