...
rasch_fit <- irt_dat%>%
  ...
  add_epred_draws(irt1, re_formula = ~(1 | word) + (1 | subjID), ndraws = 100)%>%
...
```{r}
ggsave("../graphs/in_outfit.png", height = 8, width = 16, scale = 1.5)
```
...
disc_2PL <- coef(irt2PL)$targetWord[, , "logalpha_Intercept"] %>% as_tibble(rownames = "item")%>%pull(Estimate)
