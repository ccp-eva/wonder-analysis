...
irt1 <- add_criterion(irt1,criterion = c("loo", "waic")) 
...
irt2PL <- add_criterion(irt2PL,criterion = c("loo", "waic")) 
...
```{r}
persparams <- ranef(irt1)$subjID%>%as_tibble(rownames = "subjID")%>%mutate(model = "full")

ggplot(persparams, aes(x = Estimate.Intercept, y = reorder(subjID, Estimate.Intercept)))+
  geom_pointrange(aes(xmin = Q2.5.Intercept, xmax = Q97.5.Intercept), alpha = 0.2)+
  theme_minimal()+
...
```{r}
sim_rasch_test <- simulated_annealing_rasch(10)

sel_items_rasch_test <- items[unlist(sim_rasch_test$best_subset) == TRUE]


sel_items <- sample(unique(data$word), 10)

sub_subj <- sample(unique(data$subjID), 10)
...
irt1_small <- add_criterion(irt1_small,criterion = c("loo", "waic")) 
...
irt2PL_small <- add_criterion(irt2PL_small,criterion = c("loo", "waic")) 


loo_compare(irt1_small, irt2PL_small)
