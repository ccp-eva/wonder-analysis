...
irt_dat <- data%>%
  select(subjID, english, score, sex, aoa_rating_german)

aoa <- data%>%distinct(english, .keep_all = T)%>%select(english, aoa_rating_german)
...
prior_1pl <- 
  ...
  prior("normal(0, 3)", class = "sd", group = "english")
```
...
rasch_fit <- irt_dat%>%
  ...
  add_epred_draws(irt1, re_formula = ~(1 | english) + (1 | subjID), ndraws = 5000)%>%
  mutate(zvi = (correct - .epred)/(.epred*(1-.epred))^0.5)%>%
  group_by(english,aoa_rating_german, .draw)%>%
...
rasch_fit_mode <- rasch_fit%>%
  ...
  group_by(english, aoa_rating_german, fit_index)%>%
...
```{r}
rasch_fit_mode%>%
  ggplot(. , aes(y = fct_reorder(english, aoa_rating_german), x = mode, col = fit_index))+
