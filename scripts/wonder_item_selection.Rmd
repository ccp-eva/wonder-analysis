...
fit_selected_items <- rasch_fit_mode%>%ungroup()%>%filter(0.5 < mode & 1.5 > mode)%>%group_by(word)%>%mutate(n = n())%>%filter(n == 2)%>%distinct(word)%>%pull(word)
...
items_out <- c("lieb","Mütze","Tür", "Handschuh" )
  
all_items <- irt_dat%>%
  filter(!word %in% all_solve_items)%>%
  filter(!word %in% items_out)%>%
  distinct(word)%>%
  pull(word)
...
lavaan::summary(freqAll, standardized = TRUE)

miAll <- modindices(freqAll)
saveRDS(miAll, "../saves/miAll.rds")
